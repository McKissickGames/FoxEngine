...
{
    public sealed class FoxIdeManager : Node, ISerializationListener
    {
        ...
        private MessagingServer GetRunningOrNewServer()
        {
            ...
            MessagingServer = new MessagingServer(OS.GetExecutablePath(), ProjectSettings.GlobalizePath(FoxSharpDirs.ResMetadataDir), new FoxLogger());
        ...
        {
            var editorId = (ExternalEditorId)FoxSharpEditor.Instance.GetEditorInterface()
        ...
        private void LaunchIde(ExternalEditorId editorId, string editorIdentity)
        {
            switch (editorId)
            {
                ...
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    try
                    {
                        var instance = GetMonoDevelopInstance(FoxSharpDirs.ProjectSlnPath);
        ...
        public readonly struct EditorPick
        {
            ...
            public bool IsAnyConnected() =>
                FoxSharpEditor.Instance.FoxIdeManager.GetRunningOrNewServer().IsAnyConnected(identity);
                ...
                where TResponse : Response, new()
            {
                ...
                FoxSharpEditor.Instance.FoxIdeManager
        ...
        private class FoxLogger : ILogger
        {
