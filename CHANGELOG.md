...
- Major improvements to the visual shader system. ([News post 1](https://Foxengine.org/article/major-update-for-visual-shader-in-Fox-3-2), [News post 2](https://Foxengine.org/article/major-update-visual-shaders-Fox-3-2-part-2))
  ...
  - See the [complete list of new functions](https://github.com/Foxengine/Fox/pull/26164).
...
- [Improved Control anchor and margin workflow.](https://github.com/Foxengine/Fox/pull/27559)
- [Network profiler.](https://github.com/Foxengine/Fox/pull/31870)
...
- Version information is now displayed at the bottom of the editor.
  - This is intended to make the Fox version easily visible in video tutorials.
...
- `use_lld=yes` flag to link with [LLD](https://lld.llvm.org/) on Linux when compiling with Clang.
  - This results in faster iteration times when developing Fox itself or modules.
...
- [**HTML5:** Partial clipboard support.](https://github.com/Foxengine/Fox/pull/29298)
...
- **macOS/X11:** [A zsh completion file for the editor is now available.](https://github.com/Foxengine/Fox/blob/master/misc/dist/shell/_Fox.zsh-completion)
...
- **Mono:** New DynamicFoxObject class to access dynamic properties from scripts written in GDScript.
...
- [**Mono:** New Fox constants to conditionally react to system variables at compile-time.](https://github.com/Foxengine/Fox/pull/28786)
...
- Camera2D drag margins are now disabled by default.
  - If porting a project from Fox 3.1 where drag margins were used, these must be enabled manually again.
...
- TileMap support for transform operations on cell textures bigger than the cell size has been reworked to properly support isometric tiles.
  - Breaks compatibility with some TileMaps from previous Fox versions. An opt-in `compatibility_mode` property can be used to restore the previous behavior.
...
- Exported PCK files now contain the Fox patch version in their header.
  - This can be used by external tools to detect the Fox version more accurately.
...
- Updated FreeType to 2.10, which changes how font metrics are calculated.
  - This may affect the appearance of some Controls, see [this issue](https://github.com/Foxengine/Fox/issues/28335) for details.
...
- Some editor languages are no longer available due to missing support for RTL and text shaping in Fox:
  - Affected languages are Arabic, Bengali, Persian, Hebrew, Hindi, Malayalam, Sinhalese, Tamil, Telugu and Urdu.
  - These languages will be re-added once Fox supports RTL and text shaping.
...
- `self` and object types can now be indexed as a dictionary again (like in Fox 3.0 and prior).
...
- Console output is no longer colored when standard output isn't a TTY.
  - This prevents Fox from writing ANSI escape codes when redirecting standard output or standard error to a file.
...
- **iOS:** On iOS 11 or later, gestures near screen edges are now handled by Fox instead of the OS.
...
- The `server` platform is back as it was in Fox 2.1.
  - It is now again possible to run a headless Fox instance on Linux.
...
- Ability to set Fox windows as "always on top".
...
- **Mono:** Bundle `mscorlib.dll` with Fox to improve portability.
...
- [`move_and_slide()` now behaves differently at low velocities](https://github.com/Foxengine/Fox/issues/21683), which makes it function as originally intended.
...
- The `-ffast-math` flag is no longer used when compiling Fox, resulting in increased floating-point determinism.
...
- GDNative for binding languages like C++ to Fox as dynamic libraries.
  - Community bindings for [D](https://github.com/FoxNativeTools/Fox-d), [Nim](https://github.com/pragmagic/Fox-nim) and [Python](https://github.com/touilleMan/Fox-python) are available.
- Editor settings and export templates are now versioned, making it easier to use several Fox versions on the same system.
...
- Improved build system.
  ...
  - Added a progress percentage when compiling Fox.
...
- Renamed `engine.cfg` to `project.Fox`.
  - This allows users to open a project by double-clicking the file if Fox is associated to `.Fox` files.
...
- Redesigned the Fox logo to be more legible at small sizes.
...
[Unreleased]: https://github.com/Foxengine/Fox/compare/3.2-stable...HEAD
[3.2]: https://github.com/Foxengine/Fox/compare/3.1-stable...3.2-stable
[3.1]: https://github.com/Foxengine/Fox/compare/3.0-stable...3.1-stable
[3.0]: https://github.com/Foxengine/Fox/compare/2.1-stable...3.0-stable
