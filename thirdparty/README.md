...
- All cpp files listed in `modules/raycast/Fox_update_embree.py`
- All header files in the directories listed in `modules/raycast/Fox_update_embree.py`

The `modules/raycast/Fox_update_embree.py` script can be used to pull the
relevant files from the latest Embree release and apply some automatic changes.

Some changes have been made in order to remove exceptions and fix minor build errors.
They are marked with `// -- Fox start --` and `// -- Fox end --`
...
to be usable by Fox's socket implementation and allow IPv6 and DTLS.
Apply the patches in the `patches/` folder when syncing on newer upstream
commits.

Three files (Fox.cpp, enet/Fox.h, enet/Fox_ext.h) have been added to provide
enet socket implementation using Fox classes.
...
- the `icudt69l.dat` built with the provided `Fox_data.json` config file (see
...
Important: Some files have Fox-made changes.
They are marked with `// -- Fox start --` and `// -- Fox end --`
comments.
...
Fox-made change marked with `// -- Fox --` comments.
...
Important: The files `utils/bit_reader_utils.{c,h}` have Fox-made
changes to ensure they build for Javascript/HTML5. Those
changes are marked with `// -- Fox --` comments.
...
- Added 2 files `Fox_core_mbedtls_platform.{c,h}` providing configuration
...
The only modified file is `miniupnpcstrings.h`, which was created for Fox
...
Important: Some files have Fox-made changes for use in core/io.
They are marked with `/* Fox start */` and `/* Fox end */`
...
Collection of single-file libraries used in Fox components.

- `clipper.{cpp,hpp}`
  * Upstream: https://sourceforge.net/projects/polyclipping
  * Version: 6.4.2 (2017) + Fox changes (added optional exceptions handling)
  * License: BSL-1.0
- `cubemap_coeffs.h`
...
- `easing_equations.cpp`
  * Upstream: http://robertpenner.com/easing/ via https://github.com/jesusgollonet/ofpennereasing (modified to fit Fox types)
  * Version: git (af72c147c3a74e7e872aa28c7e2abfcced04fdce, 2008) + Fox types and style changes
  * License: BSD-3-Clause
- `fastlz.{c,h}`
...
- `polypartition.{cpp,h}`
  ...
  * Modifications: Change from STL to Fox types (see provided patch).
  * License: MIT
- `r128.h`
...
Modified files:
Modifications are marked with `// -- Fox start --` and `// -- Fox end --`.
...
Important: Some files have Fox-made changes; so to enrich the features
originally proposed by this library and better integrate this library with
Fox. Please check the file to know what's new.
...
`// -- Fox begin --` and `// -- Fox end --` comments.
...
Some downstream changes have been made and are identified by
`// -- Fox start --` and `// -- Fox end --` comments.
...
Version: 2.3.0 (2019)
`vk_mem_alloc.cpp` and `android/vk_mem_alloc.cpp` are Fox files and should be preserved on updates.
...
- `wslay.h` has a small Fox addition to fix MSVC build.
